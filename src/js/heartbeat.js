 ...
 *
 * @return {Promise<{login:boolean, success:boolean, message:string, code: string}>} User information object.
 */
...
CookieTasting.confirm = ( shouldRefresh = false ) => {
	...
	if ( ! CookieTasting.shouldConfirm() ) {
		CookieTasting.log(
			'No need to confirm: ' + now.toLocaleString(),
	...
	CookieTasting.refreshNonce()
		...
		.finally( () => {
			...
			CookieTasting.log(
				...
				CookieTasting.lastUpdated(),
				Math.floor( finished.getTime() / 1000 )
			);
		} );
};
